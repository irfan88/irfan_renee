...
contract Irfan is ERC721URIStorage, AccessControl, ERC721Enumerable, Ownable {
    ...
    address public ADMIN_WALLET = msg.sender;

    bool public isAdminMinted = false;
    constructor() ERC721("Chatter", "Cht") {
        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _setupRole(MINTER_ROLE, msg.sender);
        setBaseURI("https://ipfs.io/");
        MAXIMUM_TOTAL_SUPPLY = 1111;
        PRE_SALE_TIME_START = 1636734032; // 6 PM UTC Time
        PRE_SALE_TIME_END = 1636906832; // 9 PM UTC Time
        PUBLIC_SALE_TIME_START = 1636734032; // 9:30 PM UTC TIME
    ...
    returns (bool)
    {
        require(preSaleWhiteListAddresses[msg.sender], "IRFAN, You are not whitelisted to mint pre sale token");
        require(!preSaleAddressesMint[msg.sender], "IRFAN, You have already minted pre sale token");
        require(block.timestamp >= PRE_SALE_TIME_START && block.timestamp <= PRE_SALE_TIME_END, "CHATTER: This is not pre sale time for minting");
        require(totalSupply() + 1 <= MAXIMUM_TOTAL_SUPPLY, "IRFAN: Maximum Supply Minted");
        require(_maxMintCount >= 0 && _maxMintCount <= 1, "IRFAN: Invalid Max Mint Count");
        require(msg.value == MINT_NFT_FEE, "CHATTER: Invalid Minting Fee");
